            ...
            <div class="inside">
              <form method="POST" action="" name="acumbamail_config_form">
                ...
                <p><label for="acumba_customer_id">Customer ID</label><br>
                  <input name="acumba_customer_id" id="acumba_customer_id" type="text" value="<?php if(isset($acumba_customer_id)){echo $acumba_customer_id;}?>" class="regular-text" /></p>
                <p><label for="acumba_auth_token">Auth Token</label><br>
                  <input name="acumba_auth_token" id="acumba_auth_token" type="text" value="<?php if(isset($acumba_auth_token)){echo $acumba_auth_token;}?>" class="regular-text"></p>

                </div> <!-- .inside -->
		<?php if((count($response_lists) == 0) && isset($acumba_customer_id)){ ?>
                	<p style="color:red;">Los datos no son válidos o aún no tiene listas para mostrar</p>
		<?php } ?>
            </div> <!-- .postbox -->
                  ...
                  <tbody>
                    ...
                    foreach ($ordered as $key => $value) {
                      ...
                      echo '<td class="row-title"><label for="tablecell">'.$widget_fields[$value]['name'].'</label></td>
                                                      ...
                                                      <td><input type="text" name="'.$widget_fields[$value]['name'].'-given" value="'.$widget_fields[$value]['name_given'].'"></td>
                                                      </tr>';
                    }
                    ?>
                  </tbody>
